...
class CephadmServe:
    ...
    def serve(self) -> None:
        ...
        while self.mgr.run:
            ...
            upgrade_block_str = ' This message is to make you aware of RGW issues when upgrading from RHCS 5.1 to 5.2. Check `ceph health detail`'
            if not self.mgr.no_five_one_rgw:
                ...
                if (
                    ...
                    or '5.1_TO_5.2_UPGRADE_ISSUE_WARNING_CHECK_CEPH_HEALTH_DETAIL_FOR_MORE_INFO' not in self.mgr.health_checks
                ):
                    image_rec_str = ''
                    ...
                    alert_id = '5.1_TO_5.2_UPGRADE_ISSUE_WARNING_CHECK_CEPH_HEALTH_DETAIL_FOR_MORE_INFO'
                    alert = {
                        'severity': 'error',
                        'summary': 'Upgrade: This message is to make you aware of RGW issues when upgrading from 5.1 to 5.2. Check `ceph health detail`',
                        'count': 1,
                        'detail': ['There is an issue causing potential data unavailability when using RGW on RHCS 5.1 then upgrading to RHCS 5.2.',
                                    'If you were not using RGW in your 5.1 deployment or upgraded from a different RHCS release, please run',
                                    '"ceph config set mgr mgr/cephadm/no_five_one_rgw true" to return cephadm to normal operation and resume upgrade.',
                                    'If you were in fact using RGW in RHCS 5.1 before beginning an upgrade to RHCS 5.2,',
                                    'you can attempt to downgrade via the command "ceph cephadm fallback" followed by',
                                    '"ceph orch daemon redeploy <daemon-name> <previous-image-name>" for each ceph daemon',
                                    'showing the newer version in "ceph orch ps" (will likely be just one mgr)',
                                    'Note that the fallback command may print a traceback and you may need to exit and restart your',
                                    'cephadm shell instance if you are making use of it. These issues should be transitory as part of the downgrade.',
                                    'A fix for this issue is expected to land in RHCS 5.3 and upgrade from 5.1 to 5.3 for RGW users is expected to be supported.',
                                    f'{image_rec_str}'],
                    }
                    self.mgr.log.info('Detected RHCS 5.1 to 5.2 upgrade. Setting health warnings and upgrade failure')
                    self.mgr.upgrade.upgrade_state.error = upgrade_block_str
                    ...
                    # of the mgr back to 5.1 is possible but also things will
                    # function normally if the no_five_one_rgw setting is set to true
                    upgrade_json.pop('daemon_types', None)
                    upgrade_json.pop('hosts', None)
                    upgrade_json.pop('services', None)
                    upgrade_json.pop('total_count', None)
                    upgrade_json.pop('remaining_count', None)
                ...
                self.mgr.remove_health_warning('5.1_TO_5.2_UPGRADE_ISSUE_WARNING_CHECK_CEPH_HEALTH_DETAIL_FOR_MORE_INFO')
                self.mgr.set_health_checks(self.mgr.health_checks)
                if self.mgr.upgrade.upgrade_state is not None:
                    if self.mgr.upgrade.upgrade_state.error == upgrade_block_str:
                        if self.mgr.upgrade.upgrade_state._target_name == 'UPGRADE_BLOCKER':
                            self.mgr.upgrade.upgrade_state = None
                        else:
                            self.mgr.upgrade.upgrade_state.paused = False
                            self.mgr.upgrade.upgrade_state.error = ''
