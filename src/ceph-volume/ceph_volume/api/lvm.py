...
def _vdo_parents(devices):
    ...
    ever single device in /sys/block and if any of those devices is related to
...
def _is_vdo(path):
    """
    An VDO device can be composed from many different devices, go through each one of those
    devices and its slaves (if any) and correlate them back to /dev/mapper, and then
