...
int BucketTrimInstanceCR::operate(const DoutPrefixProvider *dpp)
{
  ...
  if (clean_info->first.layout.logs.front().layout.type == rgw::BucketLogType::Deleted) {
    if (list_results.objs.empty()) {
      ...
      yield call(new RGWRadosSetOmapKeysCR(store, rgw_raw_obj(pool, oid_prefix), bucket_entry));
      if (retcode < 0) {
        ldpp_dout(dpp, 0) << "failed to store entry: " << bucket_instance
                          << " in omap instance bucket info: "
                          << cpp_strerror(retcode) << dendl;
        return set_cr_error(retcode);
      }
    }
...
/*
int remove_deleted_bucket(const DoutPrefixProvider *dpp, optional_yield y, const std::vector<std::string> buckets)
{
  auto& pool = store->svc()->zone->get_zone_params().log_pool;
  rgw_rados_ref ref;
  int r = rgw_get_rados_ref(dpp, &rados, rgw_raw_obj(pool, oid_prefix), &ref);
