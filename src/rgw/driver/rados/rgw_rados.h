...
public:
  ...
  int update_olh(const DoutPrefixProvider *dpp,
                 ...
                 BILogHandlerT&& bilog_handler,
                 rgw_zone_set *zones_trace = nullptr,
                 bool null_verid = false, bool log_op = true);
  int clear_olh(const DoutPrefixProvider *dpp,
                RGWObjectCtx& obj_ctx,
                const rgw_obj& obj,
                RGWBucketInfo& bucket_info,
...
public:
  ...
  int cls_obj_prepare_op(const DoutPrefixProvider *dpp, BucketShard& bs, RGWModifyOp op, std::string& tag, rgw_obj& obj,
                         optional_yield y, rgw_zone_set *zones_trace = nullptr);
  ...
  int cls_obj_complete_op(const RGWBucketInfo& bucket_info, BucketShard& bs, const rgw_obj& obj, std::string& tag, int64_t pool, uint64_t epoch,
                          const rgw_bucket_dir_entry& ent, RGWObjCategory category, std::list<rgw_obj_index_key> *remove_objs,
