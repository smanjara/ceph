  ...
  if (!svc.zone->is_syncing_bucket_meta(bucket)) {
   ...
   /* remove bucket index objects asynchronously by best effort */
    ...
    do {
      ...
      ldpp_dout(dpp, 20) << "setting bucket info flag to deleted" << bucket.name << dendl;
    } while (r == -ECANCELED && ++retries < max_retries);

    if (r < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to store bucket instance info for bucket=" << bucket.name << " r=" << r << dendl;
      return r;
    }
  }
