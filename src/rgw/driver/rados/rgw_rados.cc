                                           ...
                                           jspan_context& trace, bool log_op)
{
  ...
  rgw_rados_ref ref;
  int r = store->get_obj_head_ref(rctx.dpp, target->get_meta_placement_rule(), obj, &ref);
  if (r < 0)
    return r;
  ...
  if (versioned_op && meta.olh_epoch) {
    ...
    r = store->set_olh<false>(rctx.dpp, target->get_ctx(), target->get_bucket_info(), obj, NULL, *meta.olh_epoch, real_time(), false, rctx.y, meta.zones_trace, add_log);
    if (r < 0) {
      return r;
    }
  }
                                        ...
                                        jspan_context& trace, bool log_op)
{
  ...
  RGWObjState *state;
  RGWObjManifest *manifest = nullptr;
  int r = target->get_state(dpp, &state, &manifest, false, null_yield, assume_noent);
  if (r < 0) {
    return r;
  }

  //tracepoint(rgw_rados, prepare_enter, req_id.c_str());
  r = index_op.prepare(dpp, CLS_RGW_OP_ADD, &state->write_tag, null_yield, log_op);
  ...
  if (assume_noent) {
    r = _do_write_meta(size, accounted_size, attrs, assume_noent, meta.modify_tail, state, (void *)&index_op, rctx, trace, log_op);
    if (r == -EEXIST) {
      assume_noent = false;
      r = target->get_state(dpp, &state, &manifest, false, null_yield, assume_noent);
      if (r < 0) {
        return r;
      }
    }
...
set_err_state:
  if (copy_if_newer && ret == -ERR_NOT_MODIFIED) {
    ...
    if (olh_epoch && *olh_epoch > 0) {
      ...
      ret = set_olh<false>(rctx.dpp, obj_ctx, dest_bucket_info, dest_obj, nullptr,
               ...
               jspan_context& trace)
{
  ...
  ret = write_op.write_meta(dpp, obj_size, astate->accounted_size, attrs, rctx, trace);
  if (ret < 0) {
    goto done_ret;
  }
...
int RGWRados::Object::Delete::delete_obj(optional_yield y, const DoutPrefixProvider *dpp, bool log_op)
{
  ...
  if (params.versioning_status & BUCKET_VERSIONED || explicit_marker_version) {
	...
	store->gen_rand_obj_instance_name(&marker);
      ...
      int r = store->set_olh<true>(dpp, target->get_ctx(), target->get_bucket_info(), marker,
                                           ...
                                           optional_yield y, bool log_op)
{
  ...
  int r = guard_reshard(dpp, obj, &bs, [&](BucketShard *bs) -> int {
    ...
    for (const auto& entry : entries) {
      ...
      case CLS_RGW_OLH_OP_LINK_OLH:
        ...
        if (link_epoch < olh_epoch || key.instance.empty() ||
          ...
          ldpp_dout(dpp, 20) << "apply_olh_log applying key=" << entry.key << " epoch=" << olh_epoch << " delete_marker=" << entry.delete_marker
          ...
          ldpp_dout(dpp, 20) << "apply_olh skipping key=" << entry.key<< " epoch=" << olh_epoch << " delete_marker=" << entry.delete_marker
                         ...
                         bool null_verid, bool log_op)
{
  ...
  do {
    ...
    ret = apply_olh_log(dpp, obj_ctx, *state, bucket_info, obj, state->olh_tag, log, std::move(bilog_handler), &ver_marker, y, null_verid, zones_trace, log_op);
    if (ret < 0) {
      return ret;
    }
  } while (is_truncated);
  ...
  return with_bilog<CLSRGWLinkOLH<DeleteMarkerV>>(
    [&] (auto op_issuer, auto bilog_handler) {
      ...
      for (i = 0; i < MAX_ECANCELED_RETRY; i++) {
        ...
        if (ret < 0) {
          ldpp_out(dpp, 20) << "bucket_index_link_olh() target_obj=" << target_obj << " delete_marker=" << (int)DeleteMarkerV << " returned " << ret << dendl;
          olh_cancel_modification(dpp, bucket_info, *state, olh_obj, &op_issuer.op_tag, y);
          if (ret == -ECANCELED) {
...
int RGWRados::unlink_obj_instance(const DoutPrefixProvider *dpp, RGWObjectCtx& obj_ctx, RGWBucketInfo& bucket_info, const rgw_obj& target_obj,
                                  uint64_t olh_epoch, optional_yield y, uint16_t bilog_flags, bool null_verid, rgw_zone_set *_zones_trace, bool log_op)
  ...
  return with_bilog<CLSRGWUnlinkInstance>(
    [&] (auto op_issuer, auto bilog_handler) {
      ...
      for (i = 0; i < MAX_ECANCELED_RETRY; i++) {
        ...
        ret = bucket_index_unlink_instance(op_issuer, dpp, bucket_info, target_obj, olh_tag, olh_epoch, y, bilog_flags, zones_trace, log_op);
        if (ret < 0) {
          ...
          ret = update_olh(dpp, obj_ctx, state, bucket_info, olh_obj, std::move(bilog_handler), y, zones_trace, null_verid, log_op);
          if (ret == -ECANCELED) { /* already did what we needed, no need to retry, raced with another user */
            return 0;
          }
          if (ret < 0) {
...
int RGWRados::follow_olh(const DoutPrefixProvider *dpp, const RGWBucketInfo& bucket_info, RGWObjectCtx& obj_ctx, RGWObjState *state, const rgw_obj& olh_obj, rgw_obj *target, optional_yield y)
{
  map<string, bufferlist> pending_entries;
  ...
  if (!pending_entries.empty()) {
    ...
    int ret = update_olh(dpp, obj_ctx, state, bucket_info, olh_obj, get_bilog_handler(bucket_info), y);
    if (ret < 0) {
      if (ret == -ECANCELED) {
...
{
  return cls_obj_complete_op<CLSRGWCompleteModifyOp<CLS_RGW_OP_ADD>>(bucket_info, bs, obj, tag, pool, epoch, ent, category, remove_objs, bilog_flags, zones_trace, log_op);
}
                                      ...
                                      rgw_zone_set *zones_trace)
{
  ...
  return cls_obj_complete_op<CLSRGWCompleteModifyOp<CLS_RGW_OP_CANCEL>>(bucket_info, bs, obj, tag,
			     -1 /* pool id */, 0, ent,
			     RGWObjCategory::None, remove_objs, 0, zones_trace);
}
