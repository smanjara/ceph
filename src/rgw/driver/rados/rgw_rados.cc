                       ...
                       optional_yield y)
{
  ...
  for (auto& zid: zids) {
    ldpp_dout(dpp, 10) << "looking up zone: " << zid << dendl;
  }
  ...
  for (const auto& list_result: peer_status) {
    ...
    for (; entries_iter != list_result.entries.end(); ++entries_iter) {
      ...
      ldpp_dout(dpp, 10) << "found key" << entries_iter->key.name << dendl;
      if (rgw_obj_key::oid_to_key_in_ns(entries_iter->key.name, &obj, ns)) {
        ldpp_dout(dpp, 10) <<  "returning enotempty" << dendl;
        return -ENOTEMPTY;
      }
    }
  }
...
int RGWRados::delete_bucket(RGWBucketInfo& bucket_info, std::map<std::string, bufferlist>& attrs, RGWObjVersionTracker& objv_tracker, optional_yield y, const DoutPrefixProvider *dpp, bool check_empty)
{
  ...
  if (svc.zone->is_syncing_bucket_meta(bucket)) {
    ...
    if (bs_policy) {
      ldpp_dout(dpp, 10) << "bucket policy exists. list remote zones" << dendl;
      ...
      if (r == -ENOTEMPTY) {
        ...
        // don't return. continue with bucket deletion.
      }
    }
  }
