...
next:
  ...
  if (opt_cmd == OPT::BI_LIST) {
    ...
    for (; i < max_shards; i++) {
      ...
      int ret = bs.init(bucket_info.bucket, i, index, nullptr /* no RGWBucketInfo */, dpp());
      marker.clear();
  ...
  if (opt_cmd == OPT::BI_PURGE) {
    ...
    for (int i = 0; i < max_shards; i++) {
      ...
      int ret = bs.init(bucket_info.bucket, shard_id, index, nullptr /* no RGWBucketInfo */, dpp());
      if (ret < 0) {
  ...
  if (opt_cmd == OPT::BUCKET_RESHARD) {
    ...
    RGWBucketReshard br(static_cast<rgw::sal::RGWRadosStore*>(store), bucket_info, nullptr /* no callback */);
...
#define MAX_LC_LIST_ENTRIES 100
 ...
 if (opt_cmd == OPT::BUCKET_SYNC_RUN) {
  ...
  if (opt_cmd == OPT::BILOG_TRIM) {
    ...
    ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->bilog_rados->log_trim(dpp(), bucket_info, log_layout, shard_id, start_marker, end_marker);
    if (ret < 0) {
